...
@implementation EBAppStorePayDelegate

@end

@interface EBAppStorePay()<SKPaymentTransactionObserver, SKProductsRequestDelegate>

@property (nonatomic, strong)NSString *goodsId;/**<wct20180420  商品id*/

@end

@implementation EBAppStorePay
...
- (void)dof:(SKPaymentTransaction *)transaction {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(iapPay:responseAppStorePaySuccess:error:)]) {
        
        [self.delegate iapPay:self responseAppStorePaySuccess:@{@"value":transactionReceiptString} error:nil];
    }
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(iapPay:responseAppStorePayStatusshow:error:)]) {
        
        [self.delegate iapPay:self responseAppStorePayStatusshow:@{@"value":transaction} error:nil];
    }
